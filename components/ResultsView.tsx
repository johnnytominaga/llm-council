...
export default function ResultsView({
    ...
    const handleDownloadPDF = async () => {
        ...
        messages.forEach((msg, msgIndex) => {
            if (msg.role === "user") {
                ...
                if (msg.stage1 && msg.stage1.length > 0) {
                    ...
                    msg.stage1.forEach((response: Stage1Response, idx: number) => {
                        const modelName =
                ...
                if (msg.stage2 && msg.stage2.length > 0) {
                    ...
                    if (
                        ...
                        msg.metadata.aggregate_rankings.forEach(
                            (agg: AggregateRanking, index: number) => {
                                const modelName =
                                    agg.model.split("/")[1] || agg.model;
                                addText(
                                    `#${
                                        index + 1
                                    }  ${modelName} - Avg: ${agg.average_rank.toFixed(
                                        2
                    ...
                    msg.stage2.forEach((ranking: Stage2Ranking, idx: number) => {
                        const modelName =
                        ...
                        if (
                            ...
                            addSpacing(2);
                            ranking.parsed_ranking.forEach(
                                (label: string, i: number) => {
                                    const displayName = msg.metadata
                                        ?.label_to_model?.[label]
                                        ? msg.metadata.label_to_model[
                                              label
                                          ].split("/")[1] ||
                                          msg.metadata.label_to_model[label]
                                        : label;
                                    addText(
                                        `${i + 1}. ${displayName}`,
                                        9,
                ...
                if (msg.stage3?.response) {
                    ...
                    const chairman =
                        msg.stage3.model.split("/")[1] || msg.stage3.model;
        ...
        // Save PDF
        const fileName = `${conversationTitle.replace(
            /[^a-z0-9]/gi,
            "_"
        )}_${Date.now()}.pdf`;
        pdf.save(fileName);
        ...
        return (
            <Card className="my-6">
                <CardContent className="p-6 text-center text-gray-600">
                    No completed responses yet. Results will appear here once
                    ...
                    <div key={index}>
                        {msg.role === "user" ? (
                            ...
                            <div className="space-y-4">
                                ...
                                {msg.stage1 && msg.stage1.length > 0 && (
                                    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                                        <h3 className="text-sm font-semibold text-gray-900 mb-3">
                                            Stage 1: Individual Responses (
                                            {msg.stage1.length} models)
                                        </h3>
                                        <div className="space-y-3">
                                            {msg.stage1.map(
                                                (response: Stage1Response, i: number) => (
                                                    <div
                                                        key={i}
                                ...
                                {/* Stage 2 Rankings */}
                                {msg.metadata?.aggregate_rankings &&
                                    msg.metadata.aggregate_rankings.length >
                                        0 && (
                                        <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                                            ...
                                            <div className="space-y-2">
                                                {msg.metadata.aggregate_rankings.map(
                                                    (agg: AggregateRanking, i: number) => (
                                                        <div
                                                            key={i}
